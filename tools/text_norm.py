...
#
# 每条都在全库全量命中上逐条核对过。刻意比 check_translation_spec.py 的 P1–P15
# 窄：只纳入零误报项，其余一律留在检查器里报告、不自动写盘。
# ---------------------------------------------------------------------------
RULES: tuple[tuple[str, re.Pattern[str], str, str, str], ...] = (
    # 正文里的全角＆不是 XML 转义。统一成可见文本半角 &，
    # XHTML 源码序列化为合法实体 &amp;，避免裸 & 破坏 XML。
    ("fullwidth-ampersand", re.compile("\\uFF06"), "&amp;", "一.1", "全角＆ → 半角 &（XHTML 写作 &amp;）"),
