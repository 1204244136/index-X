...
# S6_24.12.10：中文侧是创约 SS 短篇，日文侧同一作品号是画集
# （`はいむらきよたか画集４ ＲＥＢＩＲＴＨ`，正文由整页图片承载，其中含图片
# 形式收录的小说），与中文侧不是同一部作品。该画集**不进入归档**，已从
# OneDrive 日文原文与缓存撤出（源档只留在 `BW提取`，见 AGENTS.md「画集／
# 纯图册源不进入 bw 预处理」）；这里保留登记，是为了将来任一侧重新出现时
# 继续跳过配对，而不是因为它现在还能配到对。
...
TEMPLATE_EXEMPT_WORK_IDS = frozenset({
    ...
    "S0_00",
    # 无 BW 分页源，明确不处理（保持原始快照结构）。S6_24.12.10 的中文侧短篇
    # 现为单侧存在（日文画集已撤档），仍按既有裁定不做固定行模板检查。
    "S6_10.06.26",
    "S6_24.12.10",
})
