...
python tools/bw_preprocess.py 某本画集.epub                # 画集/纯图册源：整本跳过并说明理由
python tools/bw_preprocess.py --force-image-only 某本画集.epub # 确实要按文本源处理时才强制
...
- ✅ 画集／纯图册源（整本没有正文文本页、全为整页图片，如「はいむらきよたか画集４　ＲＥＢＩＲＴＨ」）**整本跳过**并打印理由：不改写、不分配表头、不重命名图片、不合并分页、不写产物；画集不进入归档，只有显式 `--force-image-only` 才强制走文本管线
...
- **画集／纯图册源整本不处理**：判定用源文件形态——整本 XHTML 没有一个 `body.p-text` 正文页（`is_content`），却存在整页图片页（`is_full_page_image`：非正文页，且正文区只有图片、没有可见文本）。`.epub` 输入在**推断作品号之前**就探测（`probe_image_only`）并打印 `[跳过]` 与理由，所以不会先按书内位置分配表头、把 `i-001.jpg` 改成半截名字；目录输入与 `--dry-run`／`--check` 走同一判定（`process_dir`／`check_dir`／`check_epub` 命中后同样不落盘）。跳过不等于「内容为空」：画集正文本就以图片承载，其中还可能含**图片形式收录的小说**（`S6_24.12.10` 日文侧「はいむらきよたか画集４　ＲＥＢＩＲＴＨ」145 页整页图片即此形态），但没有可处理的文本，也不构成中日文本配对依据（`alignment_rules.NON_PAIR_WORK_IDS` 已登记该书）。画集不进入归档：源档留在 `BW提取`，日文侧不收录该作品（该画集已从 OneDrive 日文原文与缓存撤出，缓存清单基线同步更新）；确需按文本源强制处理时加 `--force-image-only`。
...
**没有对应作品的书也要检查**：`NON_PAIR_WORK_IDS` 只用于「已确认非同一作品」的常态跳过（如 `S5_02_03` 日文是一页式固定版式；`S6_24.12.10` 日文侧是画集，已撤档、现为中文单侧）；其余单侧存在的作品，缺少对应侧也照常做单侧模板检查，报告里留一行「无日文/中文对应作品，仅单侧模板检查」。这段逻辑曾在配对循环内部，导致没有日文对应的书完全没有检查，且不报错。
...
`check_translation_spec.py` 的**可写执行器**：只落地「替换值唯一 + 1:1 字符映射 + 全库全量命中逐条核对无例外」的规则，其余一律留在检查器里报告、不自动写盘。与检查器的分工是**窄**而不是等价的——本工具的规则表刻意比 `P1`–`P15` 少。
...
**豁免名单**（`EXEMPT_BOOKS`）：按「书 + 检查项」豁免，不是整本跳过。目前只有两本、只豁免 `template`，且都不是「待修的问题书」而是**已裁定不纳入正文模板检查**的文件：`S0_00`（读前必看，非正文作品）、`S6_10.06.26`（无 BW 分页源，明确不处理）。豁免外的模板命中数为 0，因此这两本的既有结构差异不会淹没真信号；往名单里加条目等于放宽体检口径，必须同时写明依据。
