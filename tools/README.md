...
本文只说明工具入口、参数、数据流和可验证行为。作品编号、文件命名、固定行模板及 Agent 操作边界以仓库根目录 `AGENTS.md` 为唯一规范来源；下文出现的规则摘要用于解释命令效果，不另立一套规则。**本文不记录历次修订记录与验收结论**（每轮改动规模、通过哪些门禁、遗留项处置等）——这类内容记入 `docs/maintenance-records/`（长期保留的检查结论），面向发布的更新汇总记入 `docs/changelog.md`。
...
逐文件检查模板符合性（L1-L3 头部结构、L4=h1 独占或空、L5=h2 独占或空（中文 Note 等包装页可为 `<ul>`/`<ol>` 列表包装占位）、L6=正文），并对中日配对文件检查总行数、h2 位置、图片行和独立 `<br/>` 行位置是否一致。`gaiji`/`height-2em` 内嵌字形不计为图片；`S2_14-02/04/07/10/13` 因图片已文本化（中文侧把整页「材料」图片重排为样式文本行），配对检查整体豁免（行数/h2/图片行/`<br/>` 均不比对）。报告写入 `.cache/epub-work/alignment-check.tsv`。纯图片页/无正文页不适用；仅单侧存在的 EPUB、日文独有包装页不参与。
...
日文原文把飞船腹部、百货公司外墙上那块广告屏写作 `<ruby>大画面<rt>エキシビジヨン</rt></ruby>`。中文版在**有注音**处已逐点统一为 `<ruby>大屏幕<rt>Exhibition</rt></ruby>`，但日文未注音的叙述句里，中文出现了照搬日文汉字的「大画面」，以及「巨大荧幕 / 大银幕 / 巨大屏幕 / 巨型屏幕 / 巨大画面 / 大型显示屏 / 液晶屏幕 / 显示器」等异译。本工具把**日文「大画面」锚点范围内**的异译统一为「大屏幕」。

- **只处理显式映射**：逐条记录（中文文件基名、行号、原串、新串），要求文件在 `Text/` 下唯一、行号有效、原串在该行**恰好出现一次**；任一条不满足即整次预检不写盘。不做全局模糊替换。
- **范围限定**：普通叙述里的「屏幕 / 画面」（电视画面、显示器、手机屏、网吧投影等非该物件）不在范围内，不会被触碰。日文侧与原作排版（含 `<ruby>` 注音）一律不改。
- **不破坏对齐**：只改行内文字，不增删行，因此中日行数对齐与 L1–L6 模板不受影响。
- **幂等**：重复运行识别已统一行并报告「跳过」，不重复改写。
- 可用 `--cache` 指定中文缓存根目录；默认 `.cache/epub-work/chinese-text`。写入保留原 BOM 与换行风格。

新增异译时应把条目追加到 `ENTRIES`，不要改成按关键词批量替换。

### 科学サイド／魔術サイド 中文术语统一（写缓存）
...
日文原文用**注音**区分两层：「サイド」全库 973 处裸写（仅 3 处带注音），是术语，指科学与魔法的两大阵营；「側」全部带指示代词注音（こちら／あちら／むこう／こっち／そちら／かれら／おれたち／あなたがた／がわ），是指示代词，指"这一方／那边"。中文版却把术语层「サイド」译成了 阵营／势力／侧／方／世界 等写法并跨卷成段漂移（旧约 10–18 卷"势力"段、创约 8–11 卷"侧"段等），本工具把术语层统一到覆盖率最高的「阵营」。

- **分层，不是一刀切**：日文「サイド」→「科学阵营／魔法阵营」；日文写汉字「側」时中文「X侧」视为对应「側」而**保留**（日文同行出现「科学側／魔術側」即触发保护），不并入术语层。指示层被升格成「阵营」的另按批 2 回落为「侧」。
- **只处理显式映射**：映射在 `tools/side_term_overrides.json`，逐条记录（中文文件基名、行号、原串、新串），要求文件在 `Text/` 下唯一、行号有效、原串在该行**恰好出现一次**；任一条不满足即整次预检不写盘。不做全局模糊替换。同一行同词多次出现时，原串带上下文（如「一半的科学势力＋魔法」）以保持唯一。
- **词表用正则**：排除假阳性——「科学方法／科学方面」里的「科学方」、「科学阵营」里的「科学营」都不算异译。
- **风格化行跳过**：带「符号 + ruby 注音」的行（创约 11 卷 CRC 口吻，如 `<ruby>〼<rt>是</rt></ruby>`、`<ruby>◎<rt>若</rt></ruby>`，其繁简混用是风格的一部分）默认不动，需 `--include-style`。
- **不破坏对齐**：只改行内文字，不增删行，中日行数对齐与 L1–L6 模板不受影响；幂等，重复运行报告「已统一」。
- 可用 `--cache` 指定中文缓存根目录、`--table` 指定映射表；默认 `.cache/epub-work/chinese-text` 与 `tools/side_term_overrides.json`。写入保留原 BOM 与换行风格。

新增异译时应把条目追加到 `side_term_overrides.json`，不要改成按关键词批量替换。

配套门禁检查（中日配对，只读）：

```powershell
python tools/check_side_terms.py
```

报「日文 サイド ↔ 中文非阵营」（error）与「日文 側 ↔ 中文用了阵营」（warning）两类违规，输出 `.cache/epub-work/side-term-check.tsv`；有 error 时返回非 0，可作发布门禁（`--no-fail` 只报告）。日文锚点若无中文配对文件（中文未收录）只统计不报错。

**剥离标签时必须先去掉 `<rt>…</rt>` 注音内容**——日文「魔術サイド」常写作 `<ruby>魔術<rt>まじゆつ</rt></ruby>サイド`，只去标签会残留注音假名，导致「魔術サイド」漏检。这两个工具与 `epub_audit.py` 均已按此口径处理。

### 妹達（シスターズ）单复数汉字统一（写缓存）
...
日文原文 `<ruby>妹達<rt>シスターズ</rt></ruby>` 全库 **389 处**（362 行、94 文件、36 部）。按规范，日文有特殊注音时译文必须有对应注音，因此两侧注音都是 `Sisters`，需要判定的只有**汉字**写「妹妹」还是「妹妹们」。判定以原文指代为锚、逐处核对（完整依据与清单见 `docs/maintenance-records/sisters-plural-number-ruling-2026-09-12.md`）：

- **单数指代 38 处**（场景中只有一名个体，或「妹達の一人」「妹達＋検体番号」）→ 汉字保持「妹妹」；
- **复数但带数量·集合修饰 82 处**（一万人の／全ての／他の／全妹達 等）→ 汉字保持「妹妹」（中文「们」不与数量词共现，避免「两万名妹妹们」）；
- **其余复数指代 269 处** → 汉字写「妹妹们」。

- **只处理显式映射**：映射在 `tools/sisters_plural_overrides.json`，逐条记录（中文文件基名、行号、第几处、原串、新串、日文行内「妹達」处数）；要求文件基名在缓存中唯一、行号有效、该行 `妹妹<Sisters>`＋`妹妹们<Sisters>` 总数等于日文处数、第 `occ` 处当前恰为 `old`，任一条不满足即整次预检不写盘。
- **按处替换，不整行改写**：同一行可能混有「该改」与「不该改」的处（日文字面相同、指代不同），工具按出现序号定位，只替换指定处。
- **不破坏注音与对齐**：`<rt>Sisters</rt>` 一律不动，只改 `ruby` 汉字；不增删行，中日行数对齐与 L1–L6 模板不受影响；幂等，重复运行报告「已符合规则」。
- **注音有无要回 BW 源核对**：缓存区的日文 XHTML 是预处理产物，`bw_preprocess.py` 会把多段 ruby 合并为单段（如 `<ruby>妹達<rt>シスターズ</rt>全<rt>ぜん</rt>体<rt>たい</rt></ruby>` → `<ruby>妹達全体<rt>シスターズぜんたい</rt></ruby>`）。判断某处原文是否带特殊注音（进而决定译文是否必须带注音）时，必须读 BW 提取源目录下的 `.epub` 原文核对，不能只看缓存。源里 BookWalker 对多数汉字词都标读音，缓存只保留特殊注音，因此锚点是**特殊读音**（当て字／外来语），普通读音不构成中文注音义务。
- 可用 `--cache` / `--table` 指定中文缓存根目录与映射表；默认 `.cache/epub-work/chinese-text` 与 `tools/sisters_plural_overrides.json`。

新增判定时应把条目追加到 `sisters_plural_overrides.json`，不要改成按关键词批量替换。

- **映射以中文行号为锚**：条目按「中文文件基名 ＋ 行号 ＋ 第几处」定位，缓存重建或行结构变动后可能失效；失效条目由预检拦截（整次不写盘并报出异常行），须按本工具判定规则重建条目后再跑。

- **中文未收录的作品不在处理范围**：中文侧尚无对应作品的 14 处（`S3_14`、`S3_16`、`S6_24.06.07`）在对账中一律记为「跳过」，不报错、不改写、也不补译；日文侧保持原样。待中文侧收录后再复跑 `--apply` 覆盖（映射表未含新增位置时按判定规则补条目）。

### BW 源注音形态核对（只读）

```powershell
python tools/check_bw_ruby_anchor.py --bw "<BW 提取源目录>" --word 妹達 --cache .cache/epub-work/japanese-text
```

缓存区的日文 XHTML 是**预处理产物**：`bw_preprocess.py` 会把多段 ruby 合并为单段（`<ruby>検<rt>シリ</rt>体<rt>アル</rt>番<rt>ナン</rt>号<rt>バー</rt></ruby>` → `<ruby>検体番号<rt>シリアルナンバー</rt></ruby>`）。因此凡涉及「原文是否带注音／注音内容是什么」的判定，都要回 BW 分页源核对，不能只看缓存。

- **输出**：锚点词出现在 ruby `base` 内的形态与计数（= 该词的实际注音形态）、出现在 `rt` 内的形态与计数（= 该注音标注了哪些汉字）、正文（去注音）中的裸写处数与样例（标出是否疑似跨 ruby 边界）、以及「总出现处数 = 带注音 + 裸写」的对账。
- **`--cache`** 时对缓存日文目录做同样聚合逐项对比，源与缓存不一致时返回非 0，可用作门禁。
- **口径**：BookWalker 源对多数汉字词都标读音（`<ruby>学<rt>がく</rt>園<rt>えん</rt>都<rt>と</rt>市<rt>し</rt></ruby>`），缓存只保留**特殊注音**；故「日文有特殊注音则译文必须有注音」的锚点是源中的特殊读音（当て字／外来语），普通读音不构成译文注音义务。
- **显式传源目录**：`--bw` 必填，不在仓库里写死 OneDrive 等个人环境路径。
