...
- 画集／纯图册源不进入 bw 预处理：整本 XHTML 没有正文文本页、全为整页图片的源（如日文 `[S6_24.12.10]はいむらきよたか画集４　ＲＥＢＩＲＴＨ`，145 页整页图片）在 `bw_preprocess.py` 中整本跳过并说明理由——不改写、不分配表头、不重命名图片、不合并分页、不写产物；只有显式 `--force-image-only` 才强制走文本管线。这类书的内容并没有丢，正文本就以图片承载（还可能含图片形式收录的小说），但没有可处理的文本，也不构成中日文本配对的依据（`S6_24.12.10` 中日两侧非同一作品，已在 `alignment_rules.NON_PAIR_WORK_IDS` 登记）；画集按图片资源原样入档。
