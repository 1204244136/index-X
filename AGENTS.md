...
- `docs/`：项目说明和需要长期保留的维护记录；`docs/translation-spec.md` 是中文成品的翻译规范正文（标点、注音、译注、译名与证据边界），判定译文是否合规时以它为准。
- `tools/`：可重复使用、经过验证的维护工具。
- `.github/workflows/`：发布和自动化流程。
- `.agents/skills/`：项目级 agent skill，放需要反复执行的任务流程正文与配套模板（当前：`translation-term-unification` 译名／术语统一、`proofread-review` 校对复核）。属于项目规约、应提交；判定口径与流程在该 skill 内维护，本文件只留指针。
...
7. **跨书文本留档**：只有跨 EPUB 的文本修改才在 `docs/maintenance-records/` 新增维护记录——即一次修改落在两本及以上书籍（全库或系列级术语统一、结构迁移、对齐口径裁定等）；记录写明修改范围、涉及书籍与文件数、判定口径、统计结果、验证结果和必要样例，只保留结论与必要证据，不复制完整文本。单本 EPUB 内部的修改（同一本书内改多个文件、术语订正、译注增删、单卷结构调整、单本内的中日迁移）不留档：判定口径、统计与证据由本次提交承载，复核与回退用 `git show`／`git revert`。书籍粒度按作品计，中日两侧的同一作品算一本。
8. **文本校验与提交**：检查 `git status`、变更文件数、`git diff --stat` 并抽查文本 diff；批量校对类改动（BetweenLines 等校对工具产出并已落到 `EPUB/` 的改动）按 `.agents/skills/proofread-review/SKILL.md` 做分级与回原文核对后再提交。确认写入范围和同步结果后，直接提交本次任务相关的文本写入与维护记录。只暂存本任务涉及的文件，不纳入用户已有改动或其他任务改动，不 push。
...
- 画集／纯图册源不进入 bw 预处理：整本 XHTML 没有正文文本页、全为整页图片的源（如日文 `[S6_24.12.10]はいむらきよたか画集４　ＲＥＢＩＲＴＨ`，145 页整页图片）在 `bw_preprocess.py` 中整本跳过并说明理由——不改写、不分配表头、不重命名图片、不合并分页、不写产物；只有显式 `--force-image-only` 才强制走文本管线。这类书的内容并没有丢，正文本就以图片承载（还可能含图片形式收录的小说），但没有可处理的文本，也不构成中日文本配对的依据（`S6_24.12.10` 中日两侧非同一作品，已在 `alignment_rules.NON_PAIR_WORK_IDS` 登记）；画集**不进入归档**：源档留在 `BW提取`，日文侧不收录该作品（该画集已从 OneDrive 日文原文与缓存撤出，`.cache` 与清单基线同步更新）。
...
- 结论、范围、判定口径、统计、保留项与例外：跨 EPUB 的修订写入 `docs/maintenance-records/`，单本 EPUB 内部的修订不留档、由提交承载（判定见「维护流程」第 7 项）；禁止写入已冻结的 `docs/changelog.md`。
