...
- 术语统一以**日文写法为锚**，不做无锚点的全局替换：日文「サイド」（术语，全库 973 处裸写、仅 3 处带注音）对应中文「科学阵营／魔法阵营」；日文「側」（指示代词，全部带 こちら／あちら／むこう／こっち／そちら／かれら／がわ 等注音）对应中文「侧／那边」。两层不得互串——既不得把术语层译成「势力／侧／方／世界」等异译，也不得把指示层升格为「阵营」。同一行同时出现「サイド」与「側」时，按日文写法分别判定。统一用 `python tools/fix_side_term_variants.py --apply`（显式映射在 `tools/side_term_overrides.json`，要求文件唯一、行号有效、原串在该行恰好出现一次，预检不过不写盘），门禁用 `python tools/check_side_terms.py`。剥离标签时必须先去掉 `<rt>…</rt>` 注音内容，否则带注音的「魔術サイド」会漏检。
- 带「符号 + ruby 注音」的风格化段落（如创约 11 卷 CRC 口吻的 `<ruby>〼<rt>是</rt></ruby>`、`<ruby>◎<rt>若</rt></ruby>`，其繁简混用与符号是风格的一部分）不得按错字或繁体残留处理，术语工具默认跳过这类行。
