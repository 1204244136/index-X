...
- 日文侧规范化：头部标签跨行折叠为一行；填充 `<br/>` 删除；跨行 h1/h2 折叠为单行；`<div class="start-3em/start-5em">` 容器内嵌标题按语义重建为独立 `<h1>`；p 型标题（`font-1em10/30`、裸 `<p>あとがき`、`<p>译注` 等）转为 `<h1>`；数字小节 `<p>N</p>` 转为 `<h2>`。中文侧裸 `<div>` 仅作为单块排版包装时清除标签，保留带 class 的语义容器；相邻裸开闭标签形成 `<div></div>` 时删除该空行。
